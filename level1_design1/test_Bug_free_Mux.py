...
async def test_mux(dut):
    ...
    await Timer(4, units='ns')
    dut._log.info(f'sel - {sel} inp31 - {inp31} out - {int(dut.out.value)}')
    assert dut.out.value == dut.inp31.value, f" result is incorrect: sel - {sel}  out - {int(dut.out.value)} != inp31 - {int(dut.inp31.value)}   Test Failed !!!"
